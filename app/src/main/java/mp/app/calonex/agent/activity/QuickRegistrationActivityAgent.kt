...
class QuickRegistrationActivityAgent : CxBaseActivity2() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        txt_signin.setOnClickListener {
           // startActivity(Intent(this@QuickRegistrationActivityAgent, LoginScreen::class.java))
    ...
    fun validData() {
        ...
        registrationPayload.userRoleName = "CX-Agent"
    ...
    fun registerUploadApi(bitmapUpload: Bitmap, keyValue: String, idValue: String) {
        if (NetworkConnection.isNetworkConnected(applicationContext)) {
            ...
            var file: File? = bitmapToFile(this@QuickRegistrationActivityAgent, bitmapUpload, keyValue)
            ...
            apiCallUploadSign.subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread()).subscribe({
                    ...
                    if (count == documentPayloadList.size) {
                        pb_ut_confirm!!.visibility = View.GONE
                        window.clearFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE)
                      //  startActivity(Intent(this@QuickRegistrationActivityAgent, SignupDoneActivity::class.java))
