...
class NewUserConfirmDetailScreenAgent : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_new_user_confirm_detail_screen_agent)
    ...
    private fun registerApi() {
        if (NetworkConnection.isNetworkConnected(applicationContext)) {
            ...
            val apiCall: Observable<UserRegistrationResponse3> =
                registerService.setupUserAgent(LoginScreen.registrationPayloadAgent)
            ...
            apiCall.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
                .subscribe({
                    ...
                    if (it!!.responseCode.equals(200)) {
                        ...
                        for (item in NewUserDocumentScreenAgent.documentPayloadList) {
                            // registerUploadApi(item.imgBitmap!!,item.imgKey!!,registerDetail.userCatalogID!!)
                            registerUploadApi(
                                item.imgBitmap!!,
                                item.imgKey!!,
                                LoginScreen.registrationPayloadAgent.userId!!
                            )
                        }
                    } else {
    ...
    fun registerUploadApi(bitmapUpload: Bitmap, keyValue: String, idValue: String) {
        if (NetworkConnection.isNetworkConnected(applicationContext)) {
            ...
            var file: File? =
                Util.bitmapToFile(this@NewUserConfirmDetailScreenAgent, bitmapUpload, keyValue)
            ...
            apiCallUploadSign.subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread()).subscribe({
                    ...
                    if (count == NewUserDocumentScreenAgent.documentPayloadList.size) {
                        pb_reg_confirm!!.visibility = View.GONE
                        window.clearFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE)
                        ...
                        startActivity(
                            Intent(
                                this@NewUserConfirmDetailScreenAgent,
