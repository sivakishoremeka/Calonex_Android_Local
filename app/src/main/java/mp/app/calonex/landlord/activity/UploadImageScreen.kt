...
import mp.app.calonex.landlord.activity.AddNewPropertyFirstScreen.Companion.filePropertyImageList
...
class UploadImageScreen : CxBaseActivity2() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (mArrayUri != null && mArrayUri.size != 0) {
            adapter = ImgListAdapter(this, mArrayUri, imgSelect, txtTagPic)
        ...
        btn!!.setOnClickListener {
            ...
            try {
                for (item in mArrayUri) {
                    ...
                    val file: File? = Util.getRealPathFromUri(this@UploadImageScreen, item)
                    filePropertyImageList.add(file!!)
                }
            } catch (e: Exception) {
                e.printStackTrace()
            }
